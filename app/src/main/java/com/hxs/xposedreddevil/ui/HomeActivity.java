...
//        synthesizer.release();
    ...
    private void DataInit() {
        ...
        if (PropertiesUtils.getValue(RED_FILE, "wechatversion", "").equals("8.0.32")) {
            spCenterVersion.setSelection(0);
        }
        if (spCenterVersion.getSelectedItem().equals("8.0.32")) {
            PropertiesUtils.putValue(RED_FILE, "wechatversion", "8.0.32");
        }
...
//                Toast.makeText(this, "暂不提供免root抢红包操作，如需实现请自行编译代码", Toast.LENGTH_SHORT).show();
                Toast.makeText(this, "只支持8.0.19版本，其他版本自行编译代码", Toast.LENGTH_SHORT).show();
